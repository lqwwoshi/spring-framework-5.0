...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//其实就是靠这个后置处理器实现的，只要是实现了某个**Aware类，那么在其实现
		//的某个方法中，就可以拿到各种Spring内置的对象，比如ApplicationContext
		...
		//避免你自己注册了某些类给Spring，导致Spring自己的类被替代了。
		//比如:你自己给了Spring一个BeanFactory，他是不会用的，他在这里会用Spring自己的Factory进行替换
		...
		// 如果没有则注册两个Bena，Key为"systemProperties"和"systemEnvironment"，Value为Map，
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		//这个list是在AnnotationConfigApplicationContext被定义，Spring自己扫描你加@Commpoent的是不会在
		//里面的，因为在内部不是同一个List
		//所谓手动给Spring的就是你手动调用AnnotationConfigApplicationContext.addBeanFactoryPostProcesor();
