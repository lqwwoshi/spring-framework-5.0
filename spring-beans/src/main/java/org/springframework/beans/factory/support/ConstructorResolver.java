...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		//实例化一个BeanWrapperImpl
		// 这个对象就是包装类，包装一下返回的bean
		...
		if (constructorToUse == null) {
			...
			if (explicitArgs != null) {
				...
				//那么这边同样会有值比如minNrOfArgs这里就为1，然后resolvedValues也会有值
				ConstructorArgumentValues cargs = mbd.getConstructorArgumentValues();
				resolvedValues = new ConstructorArgumentValues();
				/**
				 * 确定构造方法参数数量,假设有如下配置：
				 *     <bean id="lqw" class="com.lqw.Luban">
			...
			for (Constructor<?> candidate : candidates) {
				 ...
				 * 首先假设 argsToUse = [1,"lqw"，obj]
				 * 那么回去匹配到上面的构造方法的1和5
				 * 由于构造方法1有更高的访问权限，所有选择1，尽管5看起来更加匹配
				 * 但是我们看2,直接参数个数就不对所以直接忽略、
				 * 这里的if需要注意，如果Spring在循环他排序后的构造器集合时，
				 * 发现某次循环constructorToUse不为Null了(下面会有赋值)，而且参数个数要比
				 * argsToUse(同样下面会有赋值)要少，那么代表Spring已经找到了想要的构造器，直接
				 * break即可
				 *从下面Spring自己的注释也可以看出，Spring is greedy，他会选择最多参数的构造器
				 * 进行构造，当然前提是构造器的参数在Spring工厂里都有
				 *
				 */
					...
					try {
						...
						if (paramNames == null) {
							...
							if (pnd != null) {
								//获取构造方法参数名称列表
								/**
								 * 假设你有一个（String lqw,Object index）
								 * 则paramNames=[lqw,index]
								 */
						 ...
						 * 因为只能提供字符串的参数值
						 * 故而需要进行转换
						 * argsHolder所包含的值就是转换之后的对象
						 * 相当于说从paramNames转成对应的对象
						 * 2:这个方法还会进行一定的校验，比如说如果构造参数里的某个对象
						 * Spring没有找到，那么就会抛异常，下面会有catch，那么Spring就不会
						 * 选用此构造了，里面代码比较复杂，但是本质上来讲这种校验
						 * 看主要源码就会发现就是先根据beanType
						 * 找对应bean，如果找不到，就用beanName找，还找不到就是真的没有，就会
						 * 抛异常了
						 *
						 */
				 ...
				 * argsHolder.arguments和paramTypes之间的差异就叫差异量
				 * 越小，代表越接近
				 * 记住，是跟自己比得出的差异量，然后差异量在和之前的最小差异量比,这点需要清楚
				 * 不然连比较的对象都不懂就比较糊了。。
				 * 每个参数值得类型(从Spring工厂中拿到的)与构造方法参数列表的类型直接的差异
				 * 通过这个差异量来衡量或者确定一个合适的构造方法
				 * 值得注意的是第一次循环一定会typeDiffWeight < minTypeDiffWeight，因为minTypeDiffWeight的初始值非常大
				 * 然后后面每次循环如果找到一个更小的typeDiffWeight就会进行一系列的赋值操作，比如constructorToUse，
				 * 代表将要使用的构造器，最后循环结束了使用的构造器就是当时的constructorToUse了
				 */
				int typeDiffWeight = (mbd.isLenientConstructorResolution() ?
						argsHolder.getTypeDifferenceWeight(paramTypes) : argsHolder.getAssignabilityWeight(paramTypes));
				// Choose this constructor if it represents the closest match.
				//这里为啥要清空一个ambiguousConstructors变量，看到下面就懂了
				...
				//代表有两个构造的typeDiffWeight一样，但是Spring不会因为出现这种异常情况而
				// 马上抛异常，因为循环还木有结束
				//可能存在一个比这两个相等的构造器还要小的typeDiffWeight呢
			...
			//循环结束
			//没有找到合适的构造方法，那么肯定要抛异常了，因为没法构造出Bean了呀
			if (constructorToUse == null) {
				if (causes != null) {
			...
			//如果ambiguousConstructors还存在同样会抛异常
			//因为Spring不知道用哪个才能满足你了。。也不敢随便用。。
